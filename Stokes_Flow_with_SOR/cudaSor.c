...
__global__ void KernelU(double* dev_U,double* dev_P, double* dev_ResU,double* dev_numberOfBlocksU , int* dev_N, double* dev_dx, double* dev_mu, double* dev_omega){
...
int gi;
int gj;
int rowsPerBlock=*dev_N/ *dev_numberOfBlocksU; //The local block cN
int dx=*dev_dx;
int mu=*dev_mu;
int N=dev_N;
...
//Handle Device Data 
	...
	HANDLE_ERROR(cudaMalloc((void*) &dev_u, (N-1)*N,sizeof(double)));
	HANDLE_ERROR(cudaMalloc((void*) &dev_v, (N-1)*N,sizeof(double)));
	HANDLE_ERROR(cudaMalloc((void*) &dev_p, (N-1)*(N-1),sizeof(double)));
